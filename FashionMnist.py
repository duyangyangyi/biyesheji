...
from util.functions import time_record, mls_sigmod, cur2, normal_distribution, cnn_criterion
import torch
from util import my_data
from DSTA.D_STA import Dsta
train_loader, test_loader = my_data.FashionMNIST(train_size=128, test_size=1024, num_workers=8)
...
param_range = [[64, 256], [2, 7], [1, 2], [1, 1], [1, 1], [0, 2]]

criterion = nn.NLLLoss()
print(cnn_criterion(in_size, param))
...
def fitness(new_param, old_solution=0, train=True):
    ...
    for i in range(epoch):
        ...
        if 1 < i < epoch - 1 and train and (i % 2) == 0:
            x = np.arange(1, len(accuracy[9:]) + 1) + 9
            a, b, sigma = mls_sigmod(x, accuracy[9:])
            ym = cur2(a, b, 60)
            prob = normal_distribution(old_solution, ym, sigma)
            print('据前{}个点预测值：{:.2f}, 小于最优值的概率：{:2f}%'.format((i+1)*6, ym, prob*100))
    ...
    learn_curve.append(accuracy)
    print('最终真值：{:.2f}'.format(accuracy[-1]))
    time_record(s)
...
for i in range(iteration):
    ...
    best_solution.append(dsta.best_solution)
    if (i+1) % 20 == 0:
        np.save('./save/FashionMNIST/test3/param0', param0)
        np.save('./save/FashionMNIST/test3/solution0', solution0)
        np.save('./save/FashionMNIST/test3/best_param', best_param)
        np.save('./save/FashionMNIST/test3/best_solution', best_solution)
        np.save('./save/FashionMNIST/test3/num_epochs', num_epochs)
        np.save('./save/FashionMNIST/test3/learn_curve', learn_curve)
