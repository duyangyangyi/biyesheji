...
train_loader, test_loader = my_data.cifar_mini(train_size=128, test_size=1024, num_workers=8)
...
def fitness(new_param, old_solution=0, train=True):
    ...
    for i in range(epoch):
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if (batch_idx+1) % interval == 0 and i < epoch - 1 and train:
                acc = test_net(net)
                accuracy.append(acc)

        if 1 < i < epoch - 1 and train and (i % 2) == 0:
            x = np.arange(1, len(accuracy[8:]) + 1) + 8  # test2 第二轮数据开始
            a, b, sigma = mls_sigmod(x, accuracy[8:])
            ym = cur2(a, b, 50)
...
for i in range(iteration):
    ...
    best_solution.append(dsta.best_solution)
    if (i+1) % 20 == 0:
        np.save('./save/cifar-10/test4/param0', param0)
        np.save('./save/cifar-10/test4/solution0', solution0)
        np.save('./save/cifar-10/test4/best_param', best_param)
        np.save('./save/cifar-10/test4/best_solution', best_solution)
        np.save('./save/cifar-10/test4/num_epochs', num_epochs)
        np.save('./save/cifar-10/test4/learn_curve', learn_curve)
